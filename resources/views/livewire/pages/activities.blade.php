...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        $this->tags = strlen($this->search) > 0 ?
            Activity::where('name', 'like', '%' . $this->search . '%')->get()->toArray()
            : Activity::orderBy('touched', 'desc')->limit(10)->get()->toArray();
    }

    public function addActivity($id)
    {
        Activity::where('id', $id)->increment('touched');
        History::create([
            'userid' => auth()->user()->id,
            'activityid' => $id,
        ]);
